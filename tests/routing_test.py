...
def test_userpage_access_denied(client):
    ...
    post_response = client.post("user/JD0001010004", data={"login": "time_in"}, follow_redirects=True)
...
def test_userpage_access(client):
    ...
    get_response = client.get("/user/JD0001010004", follow_redirects=True)
    post_response_logout = client.post("/user/JD0001010004", data={"signout_btn": "signout"}, follow_redirects=True)
...
def test_userdashboard_access(client):
    ...
    get_response = client.get("/user/JD0001010004/dashboard", follow_redirects=True)
    post_response_logout = client.post(
        "/user/JD0001010004/dashboard", data={"signout_btn": "signout"}, follow_redirects=True
    )
    assert post_response_logout.request.path == "/"
...
def test_admin_access_denied(client):
    ...
    post_response_user = client.post("/admin/master", data={"btn_add_user": "adding_user"}, follow_redirects=True)
    post_response_class = client.post("/admin/master", data={"btn_add_class": "adding_class"}, follow_redirects=True)
...
def test_admin_access(client):
    ...
    post_response_user = client.post("/admin/master", data={"btn_add_user": "adding_user"}, follow_redirects=True)
    post_response_class = client.post("/admin/master", data={"btn_add_class": "adding_class"}, follow_redirects=True)
    post_response_logout = client.post("/admin/master", data={"signout_btn": "signout"}, follow_redirects=True)
    assert post_response_logout.request.path == "/"
...
def test_admin_adduser_access_denied(client):
    ...
    post_response = client.post(
        "/admin/master/add_user",
        data={"Name": "Test", "Firstname": "TestFirst", "DOB": "2000-01-01", "class_in": "Testclass"},
...
def test_admin_adduser_access(client):
    ...
    get_response = client.get("/admin/master/add_user", follow_redirects=True)
    post_response_noclass = client.post(
        "/admin/master/add_user",
        data={"Name": "Test", "Firstname": "TestFirst", "DOB": "2000-01-01", "CA": "NotExistingClass"},
        follow_redirects=True,
    )
    post_response = client.post(
        "/admin/master/add_user",
        data={"Name": "Test", "Firstname": "TestFirst", "DOB": "2000-01-01", "CA": "Testclass"},
        follow_redirects=True,
    )
    post_response_logout = client.post("/admin/master/add_user", data={"signout_btn": "signout"}, follow_redirects=True)
    assert post_response_logout.request.path == "/"
    assert post_response_noclass.status_code == 200 and post_response.status_code == 200
    assert b"Klasse existiert nicht. Bitte wende dich an einen Administrator!" in post_response_noclass.data
...
def test_admin_addclass_access(client):
    ...
    post_response_existing = client.post(
        "/admin/master/add_class",
        data={
            "CA": "Test",
            "Subject_area": "TestFirst",
    ...
    get_response = client.get("/admin/master/add_class", follow_redirects=True)
    post_response_logout = client.post(
        "/admin/master/add_class", data={"signout_btn": "signout"}, follow_redirects=True
    )
    assert post_response_logout.request.path == "/"
    assert post_response.status_code == 200 and post_response_existing.status_code == 200
    assert b"Klasse existiert schon!" in post_response_existing.data
...
def test_admin_classdetails_access(client):
    ...
    post_response_wrong_ca = client.post(
        "/admin/master/class/Testclass", data={"delete_class": "delete", "CA": "Testclassss"}, follow_redirects=True
    )
    post_response_delete_ca_students = client.post(
        "/admin/master/class/Testclass", data={"delete_class": "delete", "CA": "Testclass"}, follow_redirects=True
    )
    post_response_delete_ca = client.post(
        "/admin/master/class/Test", data={"delete_class": "delete", "CA": "Test"}, follow_redirects=True
    )
    post_response_logout = client.post(
        "/admin/master/class/Testclass", data={"signout_btn": "signout"}, follow_redirects=True
    )
    assert post_response_logout.request.path == "/"
    assert post_response_wrong_ca.request.path == "/admin/master/class/Testclass"
    assert b"Du hast die Klasse falsch geschrieben!" in post_response_wrong_ca.data
    assert post_response_delete_ca_students.request.path == "/admin/master/class/Testclass"
    assert b"Die Klasse hat noch Nutzer!" in post_response_delete_ca_students.data
...
def test_admin_userdetails_access(client):
    ...
    get_response = client.get("/admin/master/user/JD0001010004", follow_redirects=True)
    post_response_wrong_uid = client.post(
        "/admin/master/user/JD0001010004", data={"delete_user": "delete", "UID": "JDd0001010004"}, follow_redirects=True
    )
    post_response_delete_uid = client.post(
        "/admin/master/user/TT0001010004", data={"delete_user": "delete", "UID": "TT0001010004"}, follow_redirects=True
    )
    post_response_logout = client.post(
        "/admin/master/user/JD0001010004", data={"signout_btn": "signout"}, follow_redirects=True
    )
    assert post_response_logout.request.path == "/"
    assert post_response_wrong_uid.request.path == "/admin/master/user/JD0001010004"
    assert b"Du hast die UserID nicht richtig geschrieben!" in post_response_wrong_uid.data
